...
class Mixed_4a(nn.Module):
    def __init__(self):
        ...
        self.branch0 = nn.Sequential(
            BasicConv2d(160, 64, kernel_size=1, stride=1, padding=1),
...
class InceptionV4(nn.Module):
    def __init__(self, num_classes=1001):
        ...
        self.features = nn.Sequential(*self.model_def)
        # self.last_linear = nn.Linear(1536, num_classes)

        # creating pointers to specific points
        self.conv1 = self.features[0].conv
        self.bn1 = self.features[0].bn
        self.relu = self.features[0].relu
        self.maxpool = nn.Sequential(
            *[self.features[1], self.features[2], self.features[3]]
        )
        self.layer1 = nn.Sequential(*[self.features[4]])
        self.layer2 = nn.Sequential(*self.features[5:10])
        self.layer3 = nn.Sequential(*self.features[10:18])
        self.layer4 = nn.Sequential(*self.features[18:22])
...
def inceptionv4(num_classes=1000, pretrained="imagenet", root="~/.encoding/models"):
    if pretrained:
        ...
        model.load_state_dict(
            model_zoo.load_url(
                settings["url"], model_dir=os.path.join(root, "inception_v4")
            ),
            strict=False,
        )
        model.features = None
