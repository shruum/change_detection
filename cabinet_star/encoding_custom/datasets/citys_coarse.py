...
class CitySegmentationCoarse(BaseDataset):
    NUM_CLASS = 19
    IGNORE_INDEX = -1
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CitySegmentationCoarse, self).__init__(
    ...
    def _preprocess(self, mask_file):
        ...
        for fname in tbar:
            ...
            mask = Image.fromarray(
                self._class_to_index(np.array(Image.open(fname))).astype("int8")
            )
            masks.append(mask)
...
def get_city_pairs(folder, split="train"):
    ...
    if split == "train" or split == "val" or split == "test":
        ...
        mask_folder = os.path.join(folder, "gtCoarse/" + split)
        ...
        train_mask_folder = os.path.join(folder, "gtCoarse/train")
        val_img_folder = os.path.join(folder, "leftImg8bit/val")
        val_mask_folder = os.path.join(folder, "gtCoarse/val")
