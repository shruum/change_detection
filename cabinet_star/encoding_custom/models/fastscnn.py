...
class _DWConv(nn.Module):
    def __init__(self, dw_channels, out_channels, stride=1, dilation=1, **kwargs):
...
class FeatureFusionModule(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dwconv = _DWConv(lower_in_channels, out_channels, 1, dilation=4)
...
class fastscnn_wrapper(BaseNet):
    def __init__(self, num_classes, backbone, aux=False, se_loss=False, **kwargs):
        ...
        self.head = FastSCNN(num_classes)
...
def get_fast_scnn(dataset="citys", **kwargs):
    model = fastscnn_wrapper(datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model


if __name__ == "__main__":
    img = torch.randn(8, 3, 256, 512)
    model = FastSCNN(19)
    outputs = model(img)
    for o in outputs:
        print(o.shape)
