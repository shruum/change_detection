...
def is_cross_text(start_loc, length, vertices):
    ...
    for vertice in vertices:
        ...
        if 0.01 <= inter / (p2.area + 1e-8) <= 0.99:
            return True
    return False
...
class custom_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512):
        ...
        self.alt_transform = transforms.Compose([transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        # print("Name: {}, Shape: tramsform_map = {}, score_map = {}, geo_map = {}, ignored_map = {}".format(self.img_files[index], transform(img).shape, score_map.shape, geo_map.shape, ignored_map.shape))
        try:
            return self.transform(img), score_map, geo_map, ignored_map
        except:
            print(
                "Problematic File ===> {}, replacing with untransformed image".format(
                    self.img_files[index]
                )
            )
            n_batch = self.alt_transform(img).shape[0]
            tmp_tensor = self.alt_transform(img)
            print(tmp_tensor.shape)
            if n_batch == 1:
                return tmp_tensor.repeat(3, 1, 1), score_map, geo_map, ignored_map
            elif n_batch > 3:
                return tmp_tensor(img)[:3], score_map, geo_map, ignored_map
