...
from text_reader.utils import CTCLabelConverter, AttnLabelConverter
...
def read(opt, image_tensors, model):
    """model configuration"""
    if "CTC" in opt.Prediction:
        converter = CTCLabelConverter(opt.character)
    else:
    ...
    # # load model
    # print('loading pretrained model from %s' % opt.saved_model)
    # model.load_state_dict(torch.load(opt.saved_model, map_location=device))50
    # # imgs = np.load("my_file.npy")
    # predict
    model.eval()
    with torch.no_grad():
...
def read_with_thresh(opt, image_tensors, model, centers=None, thresh=0.80):
    """model configuration"""
    if "CTC" in opt.Prediction:
        converter = CTCLabelConverter(opt.character)
    else:
    ...
    with torch.no_grad():
        ...
        if "CTC" in opt.Prediction:
            ...
            for pred, pred_max_prob in zip(preds_str, preds_max_prob):
                ...
                confs.append(pred_max_prob.cumprod(dim=0)[-1])
            boolarr = np.array(confs) > thresh
            preds_str = [
                pred_str for pred_str, boolval in zip(preds_str, boolarr) if boolval
            ]
            confs = [conf.item() for conf, boolval in zip(confs, boolarr) if boolval]
            if centers is not None:
                centers = [
                    center for center, boolval in zip(centers, boolarr) if boolval
                ]

    return preds_str, confs, centers
...
def demo(opt):
    ...
    image_path_list = sorted(glob.glob(opt.image_folder + "/*g"))
    imgs = np.load("my_file.npy")
    # input(imgs.shape)
    ...
    #     img -= img.mean()
    #     img = cv2.resize(img, (opt.imgW, opt.imgH))[np.newaxis, np.newaxis]
    #     if imgs is not None:
    #         imgs = np.concatenate([imgs, img], axis=0)
    #     else:
    #         imgs = img

    image_tensors = torch.tensor(imgs)
    # predict
    model.eval()
    with torch.no_grad():
        batch_size = image_tensors.size(0)
