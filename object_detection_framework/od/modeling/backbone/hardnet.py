...
class HarDNet(nn.Module):
    def __init__(
        ...
        ch = ch_list[blks - 1]
        self.base.append(
            ExtraLayers(ch) if enable_extra else nn.AdaptiveAvgPool2d((1, 1))
        )
        ...
        if pretrained:
            if hasattr(torch, "hub"):
                ...
                if arch == 68 and not depth_wise:
                    ...
                    checkpoint = (
                        "https://ping-chao.com/hardnet/hardnet68ds-632474d2.pth"
                    )
                else:
                    checkpoint = (
                        "https://ping-chao.com/hardnet/hardnet39ds-0e6c6fa9.pth"
                    )
            ...
            postfix = "DS" if depth_wise else ""
            print(
                "ImageNet pretrained weights for HarDNet%d%s is loaded"
                % (arch, postfix)
            )
...
def HarDNet39(cfg, pretrained=False, **kwargs):
    ...
    model = HarDNet(
        ...
        depth_wise=True,
        enable_extra=cfg.MODEL.BACKBONE.EXTRA,
        **kwargs
    )
...
def HarDNet68(cfg, pretrained=False, **kwargs):
    ...
    """
    model = HarDNet(
        arch=68, pretrained=pretrained, enable_extra=cfg.MODEL.BACKBONE.EXTRA, **kwargs
    )
...
def HarDNet85(cfg, pretrained=False, **kwargs):
    ...
    """
    model = HarDNet(
        arch=85, pretrained=pretrained, enable_extra=cfg.MODEL.BACKBONE.EXTRA, **kwargs
    )
