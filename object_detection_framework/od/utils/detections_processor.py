...
def convert_to_format(fmt, filename, image, boxes, labels, scores, class_names):
    if fmt == "img":
        return draw_boxes(
            image, boxes, labels, scores, class_names, width=5, alpha=0.5, fill=True
        ).astype(np.uint8)
    ...
    if fmt == "json":
        ...
        result = "\n".join(
            ["%s" % ",".join(map(str, detection)) for detection in detection_list]
        )
    elif fmt == "xml":
...
def convert_to_json(detection_list):
    ...
    for cat_id in unique_cat_id:
        ...
        per_cat_list = [
            detection for detection in detection_list if detection[0] == cat_id
        ]
        sub_cat_dict = dict()
