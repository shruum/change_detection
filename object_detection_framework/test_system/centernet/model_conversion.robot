*** Settings ***
Documentation    Facility to test model conversion and inference of Centernet.
...
*** Variables ***
${TEST_DATA}    /data/nie/teams/arl/system_tests_data/object_detection_framework/centernet
${BIN_DIR}    ${HOME_DIR}/build/bin
${OUT_DIR}    ${HOME_DIR}
${MODEL}    centernet
${CONFIG_FILE}    configs/CenterNet/voc/hardnet68_cnet512_voc0712.yaml
${TEST_OUT}    ${CURDIR}/tdo
...
Convert PyTorch model into ONNX using modeltoonnx to centernet
    [Tags]    onnx_converter
    ...
    Generated onnx model should be valid    ${MODEL}.onnx    backbone_nr=438    header_line_nr=57    nms_nr=0
...
Inference with onnx engine and trtexec_json
    ...
    ...    executor=${BIN_DIR}/trtexec_json    model=${MODEL}    threshold=0.45    test_out=${TEST_OUT}    test_data=${TEST_IN}
    Log    ${log}
    Should Be Equal As Integers    ${rc}    0
    ${expected_list}=    Create List
    Run Keyword And Continue On Failure    Check detections in json
    ...    id=000003    expected_categories=${expected_list}    results_location=${TEST_OUT}/detections
