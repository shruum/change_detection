...
class Tracker(object):
    ...
    def Update(self, detections, instance_ids, frame_id):
        ...
        Args:
            ...
            instance_ids: unique ids corresponding to each detection
        Return:
            None
        """
        ...
        for i in range(len(assignment)):
            ...
            if assignment[i] != -1:
                ...
                instance_id = instance_ids[assignment[i]]
            else:
                self.tracks[i].prediction = self.tracks[i].KF.correct(
                    np.array([[0], [0]]), 0
                )
            ...
            if len(self.tracks[i].trace) > self.max_trace_length:
                for j in range(len(self.tracks[i].trace) - self.max_trace_length):
                    ...
                    del self.tracks[i].trace_ids[j]
                    del self.tracks[i].frame_ids[j]

            self.tracks[i].trace.append(self.tracks[i].prediction)
            self.tracks[i].trace_ids.append(instance_id)
            self.tracks[i].frame_ids.append(frame_id)
