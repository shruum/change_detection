...
def load_mot(detections, instance_ids=[]):
    """
    ...
    if type(detections) is str:
        ...
        ), "only numpy arrays or *.csv paths are supported as detections."
    ...
    for i in range(1, end_frame + 1):
        ...
        if len(instance_ids) > 0:
            ids = instance_ids[idx].astype(int)
        else:
            ids = -1 * np.ones(len(scores)).astype(int)
        dets = []
        for bb, s, id in zip(bbox, scores, ids):
            # Adapt to MRCNN format for bboxes: y0, x0, y1, x1
            dets.append(
                {
                    "roi": [bb[1], bb[0], bb[3], bb[2]],
                    "score": s,
...
def interp_tracks(tracks_finished):
    ...
    for ftracks in tracks_finished:
        ...
        n_frames = ending_frame - starting_frame + 1
        interp_track = np.zeros((n_frames, 4))
        instance_ids = np.zeros((n_frames,)).astype(int)

        frames_present = []

        for fframe in ftracks:
            frame_id = fframe["frame"] - starting_frame
            interp_track[frame_id, :] = fframe["roi"]
            instance_ids[frame_id] = fframe["id"]
        ...
        frames_missing = [
            f
            for f in range(starting_frame, ending_frame + 1)
            if f not in frames_present
        ]
        frames_missing_abs = (np.array(frames_missing) - starting_frame).tolist()

        for i in range(4):
            interp_track[frames_missing_abs, i] = np.interp(
                frames_missing, frames_present, interp_track[frames_present_abs, i]
            )
        instance_ids[frames_missing_abs] = -1

        furnished_tracks.append(
            [
                {
                    "roi": interp_track[f, :].tolist(),
