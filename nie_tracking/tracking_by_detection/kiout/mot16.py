...
def main(args):
    ...
    for idx, seq in enumerate(seqs):
        if seq == "name" or seq == "":
            ...
            start = time()
            tracks = track_iou(
                detections, args.sigma_l, args.sigma_h, args.sigma_iou, args.t_min
            )
            end = time()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IOU Tracker MOT demo script. Default parameters are set to reproduce "
    ...
    parser.add_argument(
        "-o", "--res_dir", type=str, required=True, help="path to the results directory"
    )
    parser.add_argument(
        "-b",
        "--benchmark_dir",
    ...
    parser.add_argument(
        "-sl", "--sigma_l", type=float, default=0.3, help="low detection threshold"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-tm", "--t_min", type=float, default=5, help="minimum track length"
    )
