...
def setup_kf(
    ...
    """
    return KalmanFilter(
        transition_matrices=a, observation_matrices=o, initial_state_mean=[*imean, 0, 0]
    )


def track_iou(
    detections, sigma_l, sigma_iou, sigma_p, sigma_len, skip_frames=False, n_skip=3
):
    """
    ...
    for frame_num, detections_frame in enumerate(detections, start=1):
        if skip_frames and (frame_num % n_skip != 0):
            continue  # optionally skip (n_skip - 1) of each (n_skip) frames
            ...
            if len(detections) > 0:
                ...
                if active_criteria(best_match, tracks) >= sigma_iou:
                    filtered_state_mean, filtered_state_cov = tracks[1].filter_update(
                        tracks[0][-1]["cur_state"],
                        tracks[0][-1]["cur_covar"],
                        best_match["centroid"],
                    )
                    best_match["cur_state"] = filtered_state_mean
                    best_match["cur_covar"] = filtered_state_cov
                    best_match["pred_state"], best_match["pred_covar"] = tracks[
                        1
                    ].filter_update(filtered_state_mean, filtered_state_cov)
        ...
        for tracks in tracks_pending:
            if frame_num - tracks[0][-1]["frame"] > sigma_p:
                ...
                tracks[0][-1]["pred_state"], tracks[0][-1]["pred_covar"] = tracks[
                    1
                ].filter_update(
        ...
        for det in new_tracks:
            ...
            det[0][0]["pred_state"], det[0][0]["pred_covar"] = det[1].filter_update(
                det[0][0]["cur_state"], det[0][0]["cur_covar"]
            )
    ...
    tracks_finished += [
        track[0] for track in tracks_pending if len(track[0]) >= sigma_p
    ]
...
def track_iou_matlab_wrapper(
    ...
    start = time()
    tracks = track_iou(
        detections, sigma_l, sigma_iou, sigma_p, sigma_len, skip_frames, n_skip
    )
    end = time()
    ...
    for track in tracks:
        for tracklet in track:
            out += [
                float(tracklet["roi"][1]),
                float(tracklet["roi"][0]),
                float(tracklet["roi"][3] - tracklet["roi"][1]),
                float(tracklet["roi"][2] - tracklet["roi"][0]),
                float(tracklet["frame"]),
                float(id_),
            ]
        id_ += 1
