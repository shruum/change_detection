...
Original had classifier for only cifr100 using pytorch. Now it is extended to any dataset
...
### 2. dataset 
dataset_type : 'cifar' for cifar100 dataset from torchvision ; 'CRDv2' for European and Chinese traffic signs
...
```bash
Ex. for CIFAR
$ python train.py -net vgg16 -dataset_type 'cifar' -num_classes 100 -img_size 32
Ex. for CRDv2
$ python train.py -net vgg16 -dataset_type 'CRDv2' -num_classes 68 -img_size 64
```
...
### 5. Infer single image
Test the model using infer_single_image.py
```bash
$ python infer_single_image.py -net vgg16 -weights path_to_vgg16_weights_file -input input_image -dataset_type CRDv2 -print
```
